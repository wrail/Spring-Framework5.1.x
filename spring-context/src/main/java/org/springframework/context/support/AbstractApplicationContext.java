...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	 ...
	 * 存放的是自定义的BeanFactoryPostProcessor
	 * BeanFactoryPostProcessor的来源可以分为三种：
	 * 1.自己手动在ApplicationContext中add的（自定义的）
	 *     1和2都在refresh的invokeBeanFactory..中完成
	 * 			存在 List<BeanFactoryPostProcessor> regularPostProcessors
	 * 			和List<BeanDefinitionRegistryPostProcessor> registryProcessors registryProcessors中

	 * 2.Spring内部的  存在currentRegistryProcessors里
	 * 3.自己写的，但是加的是@Component注解，它会在处理注解的时候进行处理
	 */
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			//准备工作包括设置启动时间，激活容器，获取当前环境验证标记是可以解析的
			prepareRefresh();
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		refreshBeanFactory();
		//此处返回的就是DefaultListableBeanFactory
		return getBeanFactory();
	}
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		...
		/**
		 * 执行自己定义的BeanFactoryPostProcessor  例如：lifecycle.TestBeanFactoryPostProcessor
		 * 需要在AppContext中add进去（算是自定义的）
		 * getBeanFactoryPostProcessors()：获取自定义的BeanFactoryPostProcessor，加@Component也可以，但是那样（会在这里获取不到）
		 * 并不是自定义的，因为此方法获取的list是在AnnotationConfigApplicationContext中维护的一个list，需要add进去才是自定义的
		 *
		 */
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 处理aspectJ的织入，编译时织入，静态，aop和cglib是动态代理
